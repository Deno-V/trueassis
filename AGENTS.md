...
一天从几点算起，由用户设置，默认 `00:00`（自然日）。设为 `04:00` 后，凌晨四点前的时刻都归属前一天。

这条设置贯穿全系统：`today`、`yesterday`、`+3d`、任务归属、日报落在哪天、逾期判断都按它算。所以用户凌晨 2 点说“今天跑完了”会记在前一天，说“写今天的日记”生成的是前一天的日报。记录里的 `*_at` 仍是真实时刻，只用于审计。

改动日界只影响之后的判断，不重写已有记录。`query` 返回里带 `day_start`，需要解释“为什么这算昨天”时引用它。
...
日期只给一端时视为查询单日，完全不给时查今天。无日期且带 `--text` 或 `--id` 时返回匹配定义，用于定位完整 ID，此时 `mode` 为 `lookup`。

逾期与无日期任务默认带出，只在明确不想被它们干扰时才用 `--no-include-overdue`、`--no-include-undated` 关闭。

返回分区：

- `records`：文本或 ID 定位结果，只在 `lookup` 模式出现；
- `scheduled`：区间内计划执行且未完成，其中已过期的带 `is_overdue`；
...
`scheduled` 回答“这段区间原本要做什么”，`overdue` 回答“这段区间之前还欠着什么”。所以查历史区间不会把当时的未完成算成今天的欠账，查未来区间也不会误报逾期。

`missed` 必须主动汇报。`skip` 是“不用补做”，不是“不用知道”，用户连续没跑步要如实说出来。

**没被查询的分区不会出现**，`queried` 列出本次查了哪些。空的 `done` 才表示确实没完成，`done` 缺席只表示没查，不能据此说用户没有进展。

`--status` 不给时是 `pending`，只带出 `scheduled`、`overdue`、`undated`、`missed`。它的取值不是分区名，没有叫 `pending` 的分区。`open` 在此之上多给未归档 `ideas`，`archived` 只给已归档 `ideas`，`all` 给全部，`done`、`cancelled`、`missed` 各只给同名分区。因此“今天完成了什么”和写日报必须改用 `--status done` 或 `--status all`。

`--kind task` 不返回 `ideas`，`--kind idea` 只返回 `ideas`。想法给了日期区间时按归属日过滤，不给日期则不过滤。

`--overdue-days` 限制循环 `carry` 向前追溯的天数，默认一年；一次性任务的欠账不受限制。
...
不带参数即查看。只接受 `00:00`–`23:59`，非法值报错。只在初始化或用户明确要求时改动。
...
这里刻意不给 `--status`，用的就是默认的 `pending`：它只回答还欠着什么，不会把已完成的事混进来。
...
先用 `--status all` 查询当天全貌（默认的 `pending` 看不到已完成），向用户确认遗漏，再生成日报。用户随口补充的见闻、进展、情绪或明日计划全部使用 `--extra` 写入，不能只保留工具自动汇总。
